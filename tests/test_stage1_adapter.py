    ...
    def test_dispatch_then_projects_verification_and_evidence(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            script.write_text(
                ...
                "json.dump({'status':'completed','execution_envelope':{'candidate_sha':'a'*40,'execution_id':'e1'},'candidate_commit_verification':{'verified':True},'evidence_result':{'status':'complete','artifact_hash':'b'*64}},open(p,'w'))\n"
            )
            ...
            evidence = execute({"step": "evidence", "operation_key": "m:evidence", "input": {"mission": {"stage1": config}, "dispatch": dispatch, "verification": verify}})
